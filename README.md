...
information: read, manipulate and write datasets using pandas
...
information: add fields after mapping them to numbers
...
information: copy fy,.. over to fx,..
...
information: write a dotfile to file 'fname' using src,dst fields
...
information: rows where any lhs-field >= v1
...
information: print help documentation and exit
...
information: select rows where any lhs-field has a value in the rhs-list
...
information: prints information about current invocation and exits
...
information: select rows where v1 <= any lhs-field <= v2
...
information: filter rows and add columns based on a matching rule
...
information: get filter match object's data-fields gx,.. and assign to fx,..
...
information: ip lookup fy in 'table', get g1,.. & assign to fx,..
...
information: join 2+ fields using sep
...
information: keep only fx,.. fields or keep fy,.. fields & rename to fx,..
...
information: summarize on all columns except fy and concat those into fx
...
information: lower-case fields or assign lower(fy),.. to fx,..'
...
information: create (fy,fx)-map and apply to existing fx,..
...
information: replace values s<x> with null value in dst/all fields
...
information: turn fy,fyz (port, protocol numbers, eg 80,6) into a portstring like 80/tcp
...
information: r is short for read, reads 1+csv-file(s)
...
information: discard any existing dataframe and load new one from 1+ csv-files
...
information: sort by fz, groupby fy[,..] most recent records
...
information: create/modify fx or filter by fy
...
information: fx := iana service name via portstring
...
information: prints dataframe information and some rows to stderr
...
information: sums or counts rows, possibly grouped on fy,..
...
information: upper-case fields or assign upper(fy),.. to fx,..
...
information: w is short for write
...
information: write any existing dataframe in csv-format out to filename/stdout
