...
def _stmt(p, stmt):
    ...
    linenr = p.lineno(1)
    err_tok = next((t for t in p if isinstance(t, ilflex.lex.LexToken)), None)
...
def p_rule_tag_error(p):
    ...
    print('action error', [x for x in p])
    p[0] = ast_item(p, ('ERROR', 'RULE', 'TAG parse error'))
...
def parse(fh):
    ...
    ilflex.lexer.lineno = 1       # reset lineno when parsing a new file
